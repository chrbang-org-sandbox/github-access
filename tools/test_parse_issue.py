...
def body(hva="Legg til", team="Kunde A (kunde-a)", hvem="Meg selv (den som sender inn)", andre="_No response_", reason="Prosjekt X"):
    return f"### Hva\n\n{hva}\n\n### Team\n\n{team}\n\n### Hvem\n\n{hvem}\n\n### Andre brukernavn\n\n{andre}\n\n### Begrunnelse\n\n{reason}"
...
class ParseIssue(unittest.TestCase):
    ...
    def test_multi_with_self_and_others(self):
        r = pi.parse(body(hvem="Meg selv (den som sender inn), hungqt, pedarn", andre="@nyansatt, en-til"), "octocat")
        self.assertEqual(r["logins"], ["octocat", "hungqt", "pedarn", "nyansatt", "en-til"])

    def test_empty_who_means_self(self):
        self.assertEqual(pi.parse(body(hvem="_No response_"), "octocat")["logins"], ["octocat"])

    def test_dedupe(self):
        r = pi.parse(body(hvem="Meg selv (den som sender inn), octocat", andre="octocat"), "octocat")
    ...
    def test_bad_login(self):
        with self.assertRaises(SystemExit):
            pi.parse(body(andre="bad name!"), "o")
