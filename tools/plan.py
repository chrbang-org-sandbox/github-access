...
ap.add_argument("--snapshot"); ap.add_argument("--config", help="alternative access.yaml (default: access.yaml)")
...
for slug, t in want["teams"].items():
    for r in set(t["repos"]) - want["all_repos"]:
        warnings.append(f"team {slug}: repo '{r}' does not exist in the org")
    for u in t["members"] - known_logins:
        warnings.append(f"team {slug}: '{u}' is not a member of the org (must be invited first)")
for r in set(want["direct"]) - want["all_repos"]:
    warnings.append(f"repos.{r}: does not exist in the org")
...
for r in sorted(active - covered):
    warnings.append(f"active repo without team write: {r}")
...
for r in sorted(want["direct"]):
    for u in sorted(want["direct"][r]):
        if u in members:
            warnings.append(f"employee with direct grant: {r}/{u} (use a team)")
...
for u in sorted(want["owners"] - have["owners"]):
    act("owner+", f"{u} becomes org owner", "PUT", f"orgs/{org}/memberships/{u}", {"role": "admin"})
for u in sorted(have["owners"] - want["owners"]):
    act("owner-", f"{u} is downgraded from owner to member", "PUT", f"orgs/{org}/memberships/{u}", {"role": "member"})
...
for slug in list(want["teams"]):
    if slug not in have["teams"] and want["teams"][slug]["name"] in by_name:
        ...
        warnings.append(f"team {slug}: exists in GitHub as '{real}' (same name). Use '{real}' as the key in access.yaml.")
        want["teams"][real] = want["teams"].pop(slug)
for slug in sorted(set(want["teams"]) - set(have["teams"])):
    t = want["teams"][slug]
    act("team+", f"create team {t['name']} ({slug})", "POST", f"orgs/{org}/teams",
        {"name": t["name"], "description": t["description"], "privacy": t["privacy"]})
for slug in sorted(set(have["teams"]) - set(want["teams"])):
    act("team-", f"delete team {slug} ({len(have['teams'][slug]['members'])} members, {len(have['teams'][slug]['repos'])} repos)", "DELETE", f"orgs/{org}/teams/{slug}")
for slug in sorted(want["teams"]):
    ...
    if slug in have["teams"] and (w["description"] != h["description"] or w["privacy"] != h["privacy"] or name_changed):
        ...
        act("team~", f"team {slug}: {'name/' if name_changed else ''}description/privacy", "PATCH", f"orgs/{org}/teams/{slug}", body)
    for u in sorted(w["members"] - h["members"]):
        role = "maintainer" if u in w["maintainers"] else "member"
        act("member+", f"team {slug}: add {u} ({role})", "PUT", f"orgs/{org}/teams/{slug}/memberships/{u}", {"role": role})
    for u in sorted(h["members"] - w["members"]):
        act("member-", f"team {slug}: remove {u}", "DELETE", f"orgs/{org}/teams/{slug}/memberships/{u}")
    ...
    for r in sorted(set(w["repos"]) - set(h["repos"])):
        if r in want["all_repos"]:
            act("grant+", f"team {slug}: {w['repos'][r]} on {r}", "PUT", f"orgs/{org}/teams/{slug}/repos/{org}/{r}", {"permission": api_role(w["repos"][r])})
    for r in sorted(set(h["repos"]) - set(w["repos"])):
        act("grant-", f"team {slug}: remove {h['repos'][r]} on {r}", "DELETE", f"orgs/{org}/teams/{slug}/repos/{org}/{r}")
    for r in sorted(set(w["repos"]) & set(h["repos"])):
...
for r in sorted(set(want["direct"]) | set(have["direct"])):
    ...
    for u in sorted(set(w) - set(h)):
        act("direct+", f"{r}: {u} gets {w[u]} (direct)", "PUT", f"repos/{org}/{r}/collaborators/{u}", {"permission": w[u]})
    for u in sorted(set(h) - set(w)):
        act("direct-", f"{r}: remove direct {h[u]} for {u}", "DELETE", f"repos/{org}/{r}/collaborators/{u}")
    for u in sorted(set(w) & set(h)):
        if w[u] != h[u]:
            act("direct~", f"{r}: {u} {h[u]} → {w[u]} (direct)", "PUT", f"repos/{org}/{r}/collaborators/{u}", {"permission": w[u]})
...
print(f"Plan for {org}: snapshot {snap['generated_at']}, config {os.path.basename(args.config) if args.config else 'access.yaml'}")
if warnings:
    print("\nWarnings:"); [print("  ! " + w) for w in warnings]
if not actions:
    print("\nNo changes. Config and reality match."); sys.exit(0)
